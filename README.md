...
- **Chaos suite.** `npm run chaos` runs seven scenarios in temporary directories on every CI run: baseline, every collector offline, subscription CLI unavailable, a corrupted `.eml`, an XLSX failure followed by recovery, an ATS board answering HTTP 500, and more candidates than the review budget allows. Each must still leave the dated folder with an HTML report.
