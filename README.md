...
When the subscription itself is exhausted the run degrades by class instead of failing: a five-hour limit is waited out (ten-minute retries for up to 90 minutes, then the rest is deferred to the next run); a model's weekly limit steps down the configured ladder (`quotaPolicy.modelLadder`, default Fable then Opus) with an info line such as "scored by opus: fable weekly limit" instead of a mismatch warning, and the next run tries the preferred model again; an account-wide weekly limit defers every remaining posting, puts a banner with the expected reset time on the report, and can hand the run to Codex when `quotaPolicy.fallbackEngine` is `codex` and Codex is signed in. Cover letters follow the same ladder and offer Generate with Codex; the Status page has a Quota card. An expired login is its own case: the CLI's "Failed to authenticate" / "OAuth session expired" / "not logged in" notices become `auth_expired`, the nightly run defers everything it could not score (never marking it unreviewed), notes the reason in `warnings.txt`, the report banner and footer, and the Run Details, and sends the macOS notification "Claude login expired, run claude auth login". In the hub the card and the panel say "Claude session expired. Run `claude auth login --claudeai` in Terminal, then try again." (with Generate with Codex when Codex is signed in), the Connections card and the sidebar show Session expired until a call succeeds or you press Refresh, and the Status page carries a banner. Any other engine failure is shown as "Generation failed (reason); details in the hub log"; raw CLI output never reaches a page. The CLI offers no usage query, so the classes are recognized from its refusal text with a configurable regex table recorded from the installed binary.
...
- **Review budget.** `semanticMatching.maxReviewedPerRun` (default 120, `0` for no limit) caps how many local candidates go to the engine per run. Freshness comes before completeness: tonight's postings (inside the lookback window) are always ranked ahead of the backlog, and local score with a small deferral bonus only reorders within those buckets. The surplus is deferred without being marked seen and stays eligible only while its posting date (or first discovery) is within `lookbackHours` plus `deferralGraceHours` (24 + 24 hours by default); older backlog expires unscored, and Run Details counts it. When the in-window candidates exceed the budget three nights running, a warning and the Status Quota card say so and show the last seven nights of candidates against the budget. The same requisition listed on several career-site paths (Workday multi-site postings) becomes one card with alternate links and one review. The report header states the real posting window of its cards.
