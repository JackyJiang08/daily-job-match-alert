...
if (!jsonPath || !outputPath) {
  console.error('Usage: node src/report-xlsx.mjs <job-radar.json> <output.xlsx> [--verify]');
  process.exit(2);
}
...
summary.getRange('A1').values = [['Daily Job Radar']];
...
summary.getRange('A3:A7').format = { fill: '#E2E8F0', font: { bold: true, color: '#334155' } };
summary.getRange('A3:B7').format.borders = { preset: 'outside', style: 'thin', color: '#CBD5E1' };
summary.getRange('A9:B12').values = [['Role type', 'Count'], ['Internship', null], ['New grad', null], ['Entry level', null]];
summary.getRange('B10').formulas = [[`=COUNTIF('Matches'!$B$2:$B$${Math.max(2, jobs.length + 1)},"internship")`]];
summary.getRange('B11').formulas = [[`=COUNTIF('Matches'!$B$2:$B$${Math.max(2, jobs.length + 1)},"new_grad")`]];
summary.getRange('B12').formulas = [[`=COUNTIF('Matches'!$B$2:$B$${Math.max(2, jobs.length + 1)},"entry_level")`]];
summary.getRange('A9:B9').format = { fill: '#164E63', font: { bold: true, color: '#FFFFFF' } };
summary.getRange('A1:F12').format.wrapText = true;
summary.getRange('A:B').format.columnWidth = 22;

const headers = ['Source', 'Role Type', 'Posted At', 'Discovered At', 'Company', 'Title', 'Location', 'Data Score', 'AI Score', 'Best Score', 'Recommended Resume', 'Why It Matches', 'Gaps / Verify', 'Posting Link', 'Freshness Basis'];
...
if (jobs.length) {
  const rows = jobs.map(job => [
    ...
    job.dataScore, job.aiScore, null, null, (job.reasons || []).join('; '), (job.gaps || []).join('; '), job.url, job.freshnessBasis,
  ]);
  ...
  matches.getRange(`A2:O${jobs.length + 1}`).format.rowHeight = 42;
  for (let row = 2; row <= jobs.length + 1; row++) {
    matches.getRange(`J${row}`).formulas = [[`=MAX(H${row},I${row})`]];
    matches.getRange(`K${row}`).formulas = [[`=IF(H${row}>=I${row},"Data","AI")`]];
  }
  matches.tables.add(`A1:O${jobs.length + 1}`, true, 'JobMatchesTable').style = 'TableStyleMedium2';
  matches.getRange(`H2:J${jobs.length + 1}`).format.numberFormat = '0';
  matches.getRange(`J2:J${jobs.length + 1}`).conditionalFormats.add('colorScale', {
    colors: ['#FEE2E2', '#FEF3C7', '#CCFBF1'], thresholds: ['min', '50%', 'max'],
  });
}
matches.freezePanes.freezeRows(1);
matches.getRange('A:O').format.wrapText = true;
const widths = [19, 13, 18, 18, 22, 42, 22, 11, 10, 11, 18, 46, 42, 55, 24];
...
setup.getRange('A3:B9').format.wrapText = true;
setup.getRange('A:A').format.columnWidth = 22;
setup.getRange('B:B').format.columnWidth = 70;

if (verifyFlag === '--verify') {
  console.log((await workbook.inspect({ kind: 'table', range: 'Run Summary!A1:B12', include: 'values,formulas', tableMaxRows: 15, tableMaxCols: 5 })).ndjson);
