...
import { generateCoverLetter } from '../cover-letter/generate.mjs';
...
export async function generateLetter(ctx, { date, jobId, trackId, company }) {
  ...
  const result = await generateCoverLetter({ engine, inputs: { playbook, samples, track, resumeText: text, job }, io: ctx.io });
  return {
    ...result,
    jobId: id,
    date,
...
export async function saveLetter(ctx, { date, jobId, trackId, company, paragraphs, engine, model, issues = [] }) {
  const config = await ctx.loadConfig();
  ...
  try {
    saved = await ctx.letterStore.saveLetter({ date, company: companyName, markdown: letter.markdown, meta: {
      ...
      engine: engine || 'unknown', model: model || 'unknown', paragraphs: body, issues, wordCount: body.join(' ').split(/\s+/).filter(Boolean).length,
  ...
  const pdf = await (ctx.renderPdf || renderLetterPdf)(letter, pdfPath, { chromeCommand: ctx.chromeCommand ?? (config.hub?.chromeCommand || null), io: ctx.io });
  saved.record.pdf = { pages: pdf.pages, layout: pdf.layout, renderer: pdf.renderer, note: pdf.note };
  await ctx.letterStore.saveLetter({ date, company: companyName, markdown: letter.markdown, meta: saved.record });
  return {
    record: saved.record,
    slug: saved.slug,
